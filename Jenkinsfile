...
def sendMetrics(String metricName, String metricValue, Map<String, String> tags) {
  ...
  withCredentials([usernamePassword(credentialsId: 'realm-influxdb', passwordVariable: 'influx_pass', usernameVariable: 'influx_user')]) {
    sh "curl -i -XPOST 'https://influxdb.realmlab.net/write?db=realm-java' --data-binary '${metricName},${tagsString} value=${metricValue}i' --user '${env.influx_user}:${env.influx_pass}'"
  }
}
